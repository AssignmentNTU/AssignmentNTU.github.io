...
public class StaffApplication {
	...
	private static ArrayList<Staff> getList = null;
	...
	public static void main(String args[]){
		...
		StaffDatabase dbStaff = new StaffDatabase();
	...
	private static void configureHoliday(){
		...
		if(holidayOption == 1){
			...
			MovieDate holidayDate  = new MovieDate(holidayYear,holidayMonth,holidayDay);
			...
			for(int i = 0 ; i < listHoliday.size() ; i++){
				MovieDate holiday = (MovieDate)listHoliday.get(i);
	...
	private static void addMovie(){
		...
		System.out.println("\ninsert the synopsis");
		String synopsis = scan.nextLine();
		movieDate = addDateOnNewMovie(title);
		Movie movie = new Movie(title, duration, ratingPG, genre, movieDate, price, director, castList, synopsis);
		if(!movieDate.getStatus().equals("Coming Soon")){
	...
	public static void removeTheDate(){
		...
		System.out.println("\n");
		ArrayList<MovieDate> listMovie = currentMovie.getArrayListOfMovieDate();
		System.out.println("Please choose which date you want to configure");
		int indexListMovie = scan.nextInt();
		MovieDate currentDate = listMovie.get(indexListMovie-1);
	...
	private static MovieDate addDateOnNewMovie(String title){
		...
		minute = scan.nextInt();
		scan.nextLine();
		MovieDate checkHolidayDate = new MovieDate(year,month,day);
		...
		movieDate = new MovieDate(cinema,status,year, month, day, hour, minute);
		return movieDate;
	}
	...
	private static void setNewDateOfMovie(Movie chooseMovie) {
		...
		MovieDate movieDate = new MovieDate(year, month, day, hour, minute);
		System.out.println("insert status to this new Date "+movieDate.getTime());
		scan.nextLine();
		String newStatus = chooseStatusMovie();
		System.out.println("specify which cinema that will play on "+movieDate.getYearMonthDay());
		int cinemaoption = getIndexCinema();
		Cinema cinema = currentCineplex.getCinema(cinemaoption-1);
		movieDate.setStatus(newStatus);
		movieDate.setCinema(cinema);
		currentMovie.setTimeStatus(movieDate);
		System.out.println("\nYour change has been saved to our system\n");
	}
	
	private static void editTime(MovieDate currentDate){
	...
	private static String chooseStatusMovie(){
	...
	private static String chooseMovieType(){
