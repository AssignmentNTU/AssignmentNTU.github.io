...
public class Movie implements Serializable{
	...
	private ArrayList<MovieDate> scheduleMovie = new ArrayList<MovieDate>();
	private int totalTicketSales = 0 ;
	private String review = "";
	private static HolidayDatabase holidayDatabase = new HolidayDatabase();
	@SuppressWarnings("unchecked")
	private static ArrayList<MovieDate> listHoliday = holidayDatabase.readFromDatabase("HolidayDatabase.dat");
	...
	public Movie(String title, String duration , String ratingPG , String genre,
			MovieDate datemovie,double price,String director,ArrayList<String> listCast,String synopsis){
		this.title = title;
	...
	public void setTimeStatus(MovieDate datemovie){
	...
	public double getPrice(int ageGroup,MovieDate dateMovie){
		...
		MovieDate first = new MovieDate(dateMovie.getYear(),dateMovie.getMonth(),dateMovie.getDay());
		...
		if(!holiday){
			//check whether it is saturday and sunday or not
			 Calendar cal = Calendar.getInstance();
			 cal.set(Calendar.YEAR, dateMovie.getYear());
			 cal.set(Calendar.MONTH, dateMovie.getMonth()-1);
			 cal.set(Calendar.DAY_OF_MONTH, dateMovie.getDay());
			 Date dateRepresentation = cal.getTime();
			 if(dateRepresentation.getDay() == 6 || dateRepresentation.getDay() == 0){
				notificationMessage+="It is Week End, the price will be increase by 2 dollars\n";
				increasePrice+=2;
			 }
		}
	...
	public ArrayList<MovieDate> getArrayListOfMovieDate(){
		return scheduleMovie;
	}
	
	//get the date movie that was available
	public ArrayList<MovieDate> getAvailableMovieDateList(){
		//file ring the date
		ArrayList<MovieDate> returnDateList  = new ArrayList<MovieDate>();
	...
	public String printDescription(){
		...
		for(int i = 0 ; i < scheduleMovie.size() ; i++){
			if(i >= 1){
				MovieDate current = scheduleMovie.get(i);
				MovieDate before = scheduleMovie.get(i-1);
